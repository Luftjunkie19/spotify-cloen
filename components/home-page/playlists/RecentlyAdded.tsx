...
function RecentlyAdded({ closedRight }: Props) {
  ...
  return (
    <>
      ...
      {!isLoading && data && data.slice(0, 7).map((song:any, i:any) => (<Link href={`/song/${song.id}`} className={`flex hover:bg-spotifyOpacityDarkGray py-2 px-1 transition-all duration-500 rounded-lg overflow-hidden cursor-pointer group snap-always snap-center flex-col relative top-0 left-0 gap-2`} key={i}>
